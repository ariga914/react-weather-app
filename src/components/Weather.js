...
const Weather = (props) => {
    ...
    useEffect(() => {
        fetch(
            `${process.env.REACT_APP_OW_API_URL}/weather/?q=${props.city_name}&APPID=${process.env.REACT_APP_OW_API_KEY}&units=metric`
        )
            .then((res) => res.json())
            .then((result) => {
                setData(result);
    ...
    if (loading) {
        return (
            <div>
                <svg
                    className="animate-spin -ml-1 mr-3 w-5 text-blue500"
    ...
    return (
        <div className="p-4">
            <div
                className={`bg-gradient-to-r from-${props.color_name}-500 to-${props.color_name}-300
                    ...
                    <div className="pt-6 pr-6">
                        <div className="flex justify-between">
                            ...
                            <div>
                                ...
                                <p className="font-bold tracking-more-wider">
                                    {data.main.humidity}%
                                </p>
                            </div>
                        </div>
